...
void Manager::_storeRequest()
{
	...
	if (bytes > 0)
	{
		...
		{
			//std::cout << "\nVOU TENTAR GUARDAR OS HEARDERS!!\n";
			//passa headers verifica server e define um maxbody
		...
		{
			//std::cout << "\nCHUKED E VOU GUARDAR O PAYLOAD\n";
			//passa body e processa chuncks
			this->REFconnect->getRequest().setPayload(buffer, bytes);
		}
		else
		{
			//std::cout << "\nPOST E VOU GUARGAR O PAYLOAD\n";
			//passa body
			this->REFconnect->getRequest().setPayload(buffer, bytes);
		}
	}
