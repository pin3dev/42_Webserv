...
void Request::setHasHeaders(char *buffer, size_t size)
{
    ...
    /*     std::map<std::string, std::string>::iterator it =  this->_headerData.begin();
        for (; it != this->_headerData.end(); it++)
            Utils::print_invisibles(it->first); */
    ...
    if(this->isChunked())
    {
        ...
        //armazenar o chunked e verificar se chegou a o fim e mandar
    }
...
/*     std::cout << "PAYLOAD: " << _payload.size() << std::endl;
...
void Request::checkRequestContent()
{
    ...
    if (this->_method == "POST" && this->_headerData.find("Content-Length") != this->_headerData.end())
    {
...
{
    if (this->_headerData.find("Transfer-Enconding") == this->_headerData.end())
        return (false);
    std::string chunked = this->_headerData["Transfer-Enconding"];
