...
bool Utils::findChunkedEOF(const std::vector<char>& payload)
{
	...
	while(pos < payload.size())
	{
		...
		payload[pos + 1] && payload[pos + 1] == '\r' &&
		payload[pos + 2] && payload[pos + 2] == '\n' &&
		payload[pos + 3] && payload[pos + 3] == '\r' &&
		payload[pos + 4] && payload[pos + 4] == '\n')
		{
			return(true);
		}
		pos++;
...
size_t	Utils::countCurChunkedSize(const std::vector<char>& payload, size_t pos)
{
	...
	while (pos < payload.size())
	{
		if (payload[pos] == '\r' && payload[pos + 1] && payload[pos + 1] == '\n')
			break;
		pos++;
		i++;
	}
